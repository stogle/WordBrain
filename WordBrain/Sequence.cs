...
namespace WordBrain
{
    public class Sequence
    {
        ...
        private readonly Func<int, int, bool> _visited;
        ...
        public Sequence(Puzzle puzzle, WordTree wordTree)
        {
            ...
            _i = -1;
            _j = -1;
            _visited = (i, j) => puzzle.Grid[i, j] == null;
        ...
        private Sequence(Puzzle puzzle, WordTree wordTree, Sequence parent, int i, int j)
        {
            ...
            _i = i;
            _j = j;
            _visited = (i, j) => (i == _i && j == _j) || parent._visited(i, j);
        ...
        public IEnumerable<Sequence> Extend()
        {
            ...
            if (_parent == null)
            {
                for (int i = 0; i < _puzzle.Grid.Height; i++)
                {
                    ...
                    {
                        if (!_visited(i, j) && _wordTree.TryLetter(_puzzle.Grid[i, j]!.Value, out wordTree!))
                        {
            ...
            if (_i - 1 >= 0)
            {
                if (isNotFirstColumn && !_visited(_i - 1, _j - 1) && _wordTree.TryLetter(_puzzle.Grid[_i - 1, _j - 1]!.Value, out wordTree!))
                {
                    yield return new Sequence(_puzzle, wordTree, this, _i - 1, _j - 1); // NW
                }
                if (!_visited(_i - 1, _j) && _wordTree.TryLetter(_puzzle.Grid[_i - 1, _j]!.Value, out wordTree!))
                {
                    yield return new Sequence(_puzzle, wordTree, this, _i - 1, _j); // N
                }
                if (isNotLastColumn && !_visited(_i - 1, _j + 1) && _wordTree.TryLetter(_puzzle.Grid[_i - 1, _j + 1]!.Value, out wordTree!))
                {
                    yield return new Sequence(_puzzle, wordTree, this, _i - 1, _j + 1); // NE
                }
            }
            if (isNotFirstColumn && !_visited(_i, _j - 1) && _wordTree.TryLetter(_puzzle.Grid[_i, _j - 1]!.Value, out wordTree!))
            {
                yield return new Sequence(_puzzle, wordTree, this, _i, _j - 1); // W
            }
            if (isNotLastColumn && !_visited(_i, _j + 1) && _wordTree.TryLetter(_puzzle.Grid[_i, _j + 1]!.Value, out wordTree!))
            {
        ...
        public bool TryPlay(out Puzzle? puzzle)
        {
            if (_wordTree.IsWord)
            {
                Solution solution;
                if (_puzzle.Solution.TryPlay(this, out solution!))
                {
